...
class GatewayHelloEvent:
    ...
    def __repr__(self) -> str:
        return f"GatewayHelloEvent(heartbeat_interval={self.heartbeat_interval})"

class GatewayEventParser:
    def parse(data):
        raise NotImplementedError("GatewayEventParser.parse() is not implemented")

class GatewayDispatchEventParser(GatewayEventParser):
    def parse(data):
        if data["t"] == "MESSAGE_CREATE":
            return DiscordGatewayMessageCreate(data["d"])
        else:
            return None

class GatewayHelloEventParser(GatewayEventParser):
    def parse(data):
        return GatewayHelloEvent(data["d"])

class GatewayParser:
    OP_TABLE = {
        0: GatewayDispatchEventParser,
        10: GatewayHelloEventParser
    }

    def parse(data):
        if data["op"] in GatewayParser.OP_TABLE:
            return GatewayParser.OP_TABLE[data["op"]].parse(data)
        return None